...
- Visual defects found and fixed: mobile date controls squeezed by a full-width refresh button; tablet brand/language overlap; excessive stacked campaign margins; floating hourly bars; undersized onboarding time fields; QR error retry loop; onboarding favicon 404; asynchronous base dashboard overwriting Menu. Preview-only Menu now skips database reads instead of querying with the fake preview account ID.
...
- Final cache-disabled local run: 81 merchant route/width checks passed (nine routes × nine widths), all six QR codes decoded, zero JavaScript exceptions, failed requests or HTTP errors. Earlier favicon and invalid preview-account requests were fixed before this final run.
