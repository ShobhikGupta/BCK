(function(root){
  ...
  function summarize(events,coupons,{from,to,campaign,timezone},gameNames){
    const localDay=d=>timezone?root.BCKHours.parts(d,timezone).date:day(d),endDay=day(to),startDay=day(from),beforeEnd=d=>!!d&&localDay(d)<=endDay;
    const scoped=x=>!campaign||x.campaign_id===campaign,within=d=>!!d&&localDay(d)>=startDay&&localDay(d)<=endDay;
    const history=events.filter(e=>scoped(e)&&beforeEnd(e.created_at)),es=history.filter(e=>within(e.created_at));
    ...
    const completed=plays.filter(e=>e.metadata?.completed===true),red=cs.filter(c=>c.status==='redeemed'&&beforeEnd(c.redeemed_at));
    ...
    for(const e of history.filter(e=>e.visitor_id&&['scan','play','return'].includes(e.event_type))){
      const v=visitors.get(e.visitor_id)||{days:new Set(),last:0};v.days.add(localDay(e.created_at));v.last=Math.max(v.last,+new Date(e.created_at));visitors.set(e.visitor_id,v);
    }
    ...
    for(const e of es){const r=rows.get(localDay(e.created_at)),key={scan:'scans',play:'plays',win:'wins',redeem:'redeems',return:'returns'}[e.event_type];if(r&&key)r[key]++}
    for(const c of cs){const r=rows.get(localDay(c.issued_at||c.created_at));if(r)r.issued++}
